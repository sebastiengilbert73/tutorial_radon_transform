...
def main(
    ...
    #cv2.imwrite(os.path.join(outputDirectory, "simulateBga_main_density.png"), bga_density_img)
    ...
    line_integrals_arr = np.zeros((number_of_rhos, numberOfAngles), dtype=np.float32)
    center = (imageSizeHW[1]/2, imageSizeHW[0]/2)
    for thetaNdx in range(numberOfAngles):
        theta = -math.pi/2 + thetaNdx * math.pi/numberOfAngles
        for rhoNdx in range(number_of_rhos):
            rho = -rho_max + rhoNdx
            line_integral = LineIntegral(bga_density_img, rho, theta, rho_max, center)
            line_integrals_arr[number_of_rhos - rhoNdx - 1, thetaNdx] = line_integral  # Negative rho is at bottom, positive rho is at top

    cv2.imwrite(os.path.join(outputDirectory, "simulateBga_main_lineIntergrals.png"), line_integrals_arr.astype(np.uint8))

    logging.info("Computing the Radon transform the efficient way, with skimage...")
    thetas = np.linspace(0., 180., max(imageSizeHW), endpoint=False)
    sinogram = radon(bga_density_img, theta=thetas, circle=False)
    cv2.imwrite(os.path.join(outputDirectory, "simulateBga_main_sinogram.png"), sinogram)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--numberOfAngles", help="The number of angles. Default: 256", type=int, default=256)
